...
class ReportsController extends Controller
{
    ...
    {
        $f_total = 0; $f_disc = 0; $f_insurance = 0; $f_cash = 0; $f_visa = 0; $f_credit = 0; $f_epay = 0;
        ...
        $memoryToUse = 50*1024*1024*1024*1024;
        $output = fopen('php://temp/maxmemory:'.$memoryToUse, 'r+');
        $columns = array('S.No.', 'Invoice Type', 'Payment Mode', 'Customer ID', 'Customer Name', 'Invoice Number', 'Total', 'Ins Discount', 'Insurance', 'Paid As', 'Cash', 'VISA', 'Txn No', 'Credit', 'EPay', 'Invoice Date', 'Created By', 'Remark');
        ...
        $data = $table->select("$tablename.*", 'user_profiles.first_name', 'users.username', 'admins.name as admin', 'insurances.name as insurancer')
         ...
         foreach($data as $key => $index)
         {
            ...
            if($index->payment_mode == 'cash'): $cash = number_format($index->payable_amount, 3); $visa = 0; $credit = 0; $remark = ''; $epay = 0;
            elseif($index->payment_mode == 'visa'): $visa = number_format($index->payable_amount, 3); $cash = 0; $credit = 0; $remark = ''; $epay = 0;
            elseif($index->payment_mode == 'credit'): $credit = number_format($index->payable_amount, 3); $visa = 0; $cash = 0; $remark = ''; $epay = 0;
            elseif($index->payment_mode == 'epay'): $epay = number_format($index->payable_amount, 3); $visa = 0; $credit = 0; $remark = ''; $cash = 0;
            elseif($index->payment_mode == 'cash+visa'): $cash = number_format($index->payable_amount, 3); $visa = 0; $credit = 0; $remark = $index->payment_history; $epay = 0;
            else: $cash = 0; $credit = 0; $remark = ''; $epay = 0;  $visa = 0;
            endif;

            fputcsv($output, array(++$key, $itypes[$index->invoice_type], $pmode, $index->username, $index->first_name, $index->invoice_number, number_format($index->amount, 3), number_format($index->ins_disc_value, 3), number_format($ins, 3),  $index->payment_mode, $cash, $visa, $index->txn_id, $credit, $epay, date('d-m-Y H:i A', strtotime($index->created_at)), $index->admin, $remark ));
        ...
        $columns = array('', 'Total', '-', '-', '-', '-', number_format($f_total, 3), number_format($f_disc, 3), number_format($f_insurance, 3), '-', number_format($f_cash, 3), number_format($f_visa, 3), '-', number_format($f_credit, 3), number_format($f_epay, 3), '-', '-', '-');
        fputcsv($output, $columns);
    ...
    public function medicineStockDayReport($date = null)
    {
        ...
        $sep = array(' ', ' ', ' ', ' ');
        fputcsv($output, $sep);
        $columns = array('S.No.', 'Medicine', 'Out', 'Current');
        fputcsv($output, $columns);
    ...
    public function cumulativeReport($iids = null, $sdate = null, $edate = null)
    {
        ...
        foreach($insurances as $insurance)
        {
            ...
            $columns = array('S.No.', 'Client Name', 'Client Number', 'Client ID', 'Company Name', 'Policy Number', 'Expiry Date',  'Invoice Number', 'Invoice Type', 'Invoice Date', 'Net Amount', 'Claim Amount', 'Insurance Discount');
            fputcsv($output, $columns);
    ...
    public function completeInvoiceReport($date = null)
    {
        ...
        $columns = array('S.No.', 'Invoice Type', 'Invoice Number', 'Client Name', 'Client Number', 'Client ID', 'Company Name', 'Policy Number', 'Expiry Date',  'Payable Amount');
        fputcsv($output, $columns);
        ...
        foreach($invoices as $key => $index)
        {
            ...
            fputcsv($output, array(++$key, $invoice_types[$index->invoice_type], $index->invoice_number, ucwords($index->user->user_profile->first_name.' '.$index->user->user_profile->last_name), $index->user->mobile, $index->user->username, ucwords($index->user->user_profile->company_name), $index->user->user_document->policy_number, date('d-M-Y', strtotime($index->user->user_document->expiry_date)),  number_format($index->amount, 3) ));
        }
    ...
    public function monthlyCompleteInvoiceReport($date = null)
    {
        ...
        foreach($invoices as $key => $index)
        {
            ...
            fputcsv($output, array(++$key, $invoice_types[$index->invoice_type], date('d-M-Y', strtotime($index->invoice_date)), $index->invoice_number, ucwords($index->user->user_profile->first_name.' '.$index->user->user_profile->last_name), $index->user->mobile, $index->user->username, ucwords($index->user->user_profile->company_name), $index->user->user_document->policy_number, date('d-M-Y', strtotime($index->user->user_document->expiry_date)),  number_format($index->amount, 3), number_format($ins, 3), number_format($index->ins_disc_value, 3), ));
        }

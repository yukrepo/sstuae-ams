                    ...
                    <div class="row">
                        ...
                        <div class="col-md-2">
                            <select v-model="sform.doctor_id" class="form-control" @change="makeSearch">
                                ...
                                <option v-for="doctor in alldoctors" :key="doctor.id" v-bind:value="doctor.id">
                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="card">
                            ...
                            <div class="card-body p-0 m-0 table-responsive search-content-box">
                                 <table id="upcomings" class="table table-striped table-hover">
                                    ...
                                    <tbody>
                                        <tr v-for="(appointment, sn) in appointments.data" :key="appointment.id">
                                            <td>
                                                <span :class="[(appointment.is_dummy == 0)?'':'bg-warning px-1']"> 
                                                    {{ (appointments.current_page - 1)*(appointments.per_page) + sn + 1 }}
                                                </span> 
                                            </td>
                                           ...
                                           <td><button class="text-theme blank-btn text-uppercase font-weight-bold" @click="viewCustomer(appointment.user_id)">{{ appointment.first_name | capitalize}}</button>  </td>
                                            ...
                                            <td>
                                                <button class="btn btn-warning btn-sm" @click="doctorBox(appointment)"  v-show="profile.admin_type_id == 1 && appointment.is_dummy == 0">
                                                    <i class="fa fa-edit"></i>
                                                </button>
                                                <a class="btn btn-primary btn-sm" :href="'/appointments/view/'+appointment.appointment_code"><i class="fas fa-eye"></i></a>
                                                <span  v-show="profile.admin_type_id != 2 && appointment.is_dummy == 0">
                                                    <span v-show="dateInLimit(appointment.date)">
                                                        ...
                                                        <a class="btn btn-warning btn-sm" :href="'/appointments/edit-prescription/'+appointment.appointment_code" v-show="(profile.admin_type_id === 1) && (appointment.appointment_type_id == 1)"><i class="fas fa-pen"></i></a>
                                                    </span>
                                                    <span class="btn-group editbox" role="group" v-if="appointment.appointment_type_id == 1" v-show="(appointment.status_id != 5) && (appointment.status_id != 11)">
                                                        <button type="button" class="btn btn-danger btn-sm dropdown-toggle" data-toggle="dropdown">
                                                            <i class="fas fa-radiation-alt"></i>
                                                        </button>
                                                        <span class="dropdown-menu dropdown-menu-right m-0 p-0" aria-labelledby="btnGroupDrop1">
                                                            <button class="dropdown-item text-left bb-1 p-2" @click="completeAppointment(appointment.appointment_code)" v-show="(profile.admin_type_id == 1) || (profile.admin_type_id == 4)">Change Status To Complete</button>
                                                            <button class="dropdown-item text-left bb-1 p-2" @click="cancelAppointment(appointment.appointment_code)">Cancel Appointment</button>
                                                        </span>
                                                    </span>
                                                    <span class="btn-group editbox" role="group" v-show="(appointment.status_id != 5) && (appointment.status_id != 11)" v-else>
                                                        <button type="button" class="btn btn-danger btn-sm dropdown-toggle" data-toggle="dropdown">
                                                            <i class="fas fa-radiation-alt"></i>
                                                        </button>
                                                        <span class="dropdown-menu dropdown-menu-right m-0 p-0" aria-labelledby="btnGroupDrop1">
                                                ...
                                                <button class="btn btn-pink btn-sm" type="button" @click="checkFeedback(appointment)" v-show="appointment.status_id == 5 && appointment.is_dummy == 0">
...
const _todayComps = _today.getFullYear()+'-'+(_today.getMonth() + 1)+'-'+(_today.getDate()-1);
    export default {
        ...
        methods: {
            ...
            dateInLimit(date) {
                var a = moment(this.max_date);
                var b = moment(date);
                if(a.diff(b, 'days') <= 25) { return true; }
                else { return false; }
            },
            showType(){
            ...
            getResults(page = 1) {
                ...
                if(this.sform.search == 1) {
                    this.sform.post('/api/findHistoryAppointment?page=' + page)
            ...
            searchAssets() {
                ...
                axios.get('/api/getDoctorsList').then(({ data }) => (this.alldoctors = data));
                axios.get('/api/getAllTreatmentsList').then(({ data }) => (this.treatments = data));
            },
            changeYear(year) {
                this.activeYear = year;
                this.$router.push('/appointments/history/'+year);
            ...
            makeSearch() {
                ...
                this.appointments = {};
                this.sform.post('/api/findHistoryAppointment')
                    .then(({data}) => {
                        this.appointments = data;
                         this.loader = false;
                    })
            ...
            completeAppointment(id) {
                swal.fire({
                    ...
                    confirmButtonColor: '#0DC957',
                    confirmButtonText: 'Yes, Close it!',
                    cancelButtonText: 'Not Now',
                }).then((result) => {
                    if (result.value) {
                        axios.post('/api/appointments/close-appointment', {
                            appointment_code:id
                        }).then((data) => {
                                Fire.$emit('changeyear');
                            swal.fire('Completed', 'Appointment has been closed successfully.', 'success');
                        }).catch(() => {

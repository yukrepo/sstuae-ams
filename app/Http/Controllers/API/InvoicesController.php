...
class InvoicesController extends Controller
{
    ...
    public function allInvoices($year = null, Request $request)
    {
        $conditions = [];
        ...
        if($request['date']):
            return $table->select("$tablename.*", 'user_profiles.first_name', 'user_profiles.last_name', 'admins.name as admin')
                    ...
                    ->whereIn("$tablename.invoice_type", $ait)
                    ->where($conditions)
    ...
    public function createNewDirectInvoice(Request $request)
    {
        $this->validate($request, [
            'user_id' => 'required',
            'sub_total' => 'required',
        ...
        $invoice_number = $this->invoiceGenerator(9);

        $rawdata = [];

        foreach ($request['row__total'] as $key => $detail) {
            if($detail):
                $rawdata[] = ['qty' => $request['row__qty'][$key],
                            'cost' => $request['row__cost'][$key],
                            'heading' => $request['row__heading'][$key],
                            'text' => isset($request['row__text'][$key])?$request['row__text'][$key]:'',
                            'total' => $request['row__total'][$key] ];
            endif;
        }
        ...
        $invoice = DB::table($tablename)->insert([
            ...
            'payment_status' => ($request['payment_mode'] == 'credit')?1:3,
        ...
        $ptable =  new InvoicePayments;
        $ptable = $ptable->setTable('payment'.date('Y', time()).'s');
        $ptablename = $ptable->getTable();
        $payment = DB::table($ptablename)->insert([
            'location_id' => Auth::user()->location_id,
            'admin_id' => Auth::user()->id,
    ...
    public function uiInvoices($year = null)
    {
        if($year):
            $table =  new Invoices;
            $table = $table->setTable('invoice'.$year.'s');
            ...
            return $table->select("$tablename.*", 'user_profiles.first_name', 'user_profiles.last_name', 'admins.name as admin')
                    ...
                    ->join('user_profiles', 'user_profiles.user_id', '=', 'users.id')
                    ->where("$tablename.location_id", Auth::user()->location_id)
                    ->where("$tablename.invoice_type", '>', 8)
                    ->orderBy("$tablename.created_at", 'desc')->paginate(15);
        else:
            return ['message' => 'Year is missing. No data can be fetched.'];
        endif;
    ...
    public function store(Request $request)
    {
        ...
        if($aptype->appointment_type_id == 1): $sts = 8;
        else: $sts = 9; endif;
    ...
    public function storeCourseInvoice(Request $request)
    {
        ...
        $total = $request['total'];
        $subtotal = round(($total*100)/105, 2);
        $tax = $total - $subtotal;
    ...
    private function invoiceGenerator($atype = null)
    {
        ...
        $table = $table->setTable('invoice'.date('Y', time()).'s');
        $tablename = $table->getTable();
        $app = DB::table($tablename)->where('invoice_type', $atype)->orderBy('id', 'desc')->get()->first();
        if($app):
    ...
    public function addPayment(Request $request)
    {
        ...
        if($request['payment_type'] == 2):
            ...
            DB::table($tablename)->where('invoice_number', $request['invoice_number'])->update([ 'payable_amount' => $invoice->payable_amount + $request['paying_now'], 'partialpay' => json_encode($phist)]);
        elseif($request['payment_type'] == 3):
            $partialpay = ['paid_amount' => $request['paying_now'],
                            ...
                            'date' => date('Y-m-d', time())];
            array_push($phist, $partialpay);
            DB::table($tablename)->where('invoice_number', $request['invoice_number'])->update([ 'payable_amount' => $invoice->payable_amount + $request['paying_now'], 'partialpay' => json_encode($phist), 'payment_status' => 3]);
        endif;

...
class DashboardController extends Controller
{
    ...
    public function getAdminDashboardSynops()
    {
        ...
        foreach ($years as $key => $year) {
            ...
            $appointments[$key] = ['total' => $table->get()->count(),
                                    'active' => $table->get()->whereIn('status_id', [2,4,9,10,8])->count(),
                                    'completed' => $table->get()->where('status_id', 5)->count(),
                                    'deleted' => $table->get()->where('status_id', 7)->count(),
                                    'cancelled' => $table->get()->where('status_id', 11)->count(),
                                    'year' => $year->year
                                ];
        }
    ...
    {
        $scores = [];
        $doctors = Doctors::where('status_id', 2)->where('designation_type_id', 1)->get();

        $therapists = Doctors::where('status_id', 2)->where('designation_type_id', 2)->orderBy('gender', 'asc')->get();

...
class AvailabilitiesController extends Controller
{
    ...
    public function getDayShifts(Request $request)
    {
        ...
        $shift = $table->where('location_id', Auth::user()->location_id)
                        ...
                        ->get()->first();
        if($shift->type == 1):
            $shift->slots = array_values(array_unique(array_map('intval', explode(',', $shift->shift_timings))));
        else:
            $shift->slots = array_values(array_unique(array_map('intval', explode(',', $shift->block_time_slots))));
        endif;
        return $shift;
    }
    ...
    public function modifySlots(Request $request)
    {
        ...
        if($request->type == 1):
            ...
            DB::table($tablename)->where('id', $request->id)
                                ...
                                'remark' => $request['remark'],
                                'type' => $request['type'],
                                'block_time_slots' => implode(',', $slots),
                                'shift_timings' => '',
                                'status_id' => 2,
                                'hours' => $request->hours,
                                'updated_at' => Carbon::now()

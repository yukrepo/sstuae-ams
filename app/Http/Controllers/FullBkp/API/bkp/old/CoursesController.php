...
class CoursesController extends Controller
{
    ...
    public function getYearwise($year = null)
    {
        if($year):
            ...
            $tablename = $table->getTable();
            return $table->select("$tablename.*", 'statuses.status as status', 'statuses.text as status_text', 'statuses.css as status_css', 'user_profiles.first_name', 'user_profiles.last_name', 'doctors.name as doctor')
    ...
    public function getDirectYearwise($year = null)
    {
        if($year):
            ...
            $table = $table->setTable('course'.$year.'s');
            $tablename = $table->getTable();
            return $table->select("$tablename.*", 'statuses.status as status', 'statuses.text as status_text', 'statuses.css as status_css', 'user_profiles.first_name', 'user_profiles.last_name')
                        ->join('statuses', 'statuses.id', '=', "$tablename.status_id")
                        ->join('users', 'users.id', '=', "$tablename.user_id")
                        ->join('user_profiles', 'user_profiles.user_id', '=', 'users.id')
                        ->where("$tablename.status_id", '!=', 7)
    ...
    public function getPacksYearwise($year = null)
    {
        if($year):
            ...
            $table = $table->setTable('course'.$year.'s');
            $tablename = $table->getTable();
            return $table->select("$tablename.*", 'statuses.status as status', 'statuses.text as status_text', 'statuses.css as status_css', 'user_profiles.first_name', 'user_profiles.last_name', 'packages.name as package')
                        ->join('statuses', 'statuses.id', '=', "$tablename.status_id")
                        ->join('users', 'users.id', '=', "$tablename.user_id")
    ...
    private function __getInvoicesBycourses($invoices)
    {
        ...
        foreach ($invoices as $key => $value):
            ...
            $tablename = $table->getTable();
            $return[$key] = $table->select("$tablename.*", 'admins.name as admin')

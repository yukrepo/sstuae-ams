 ...
 */
    ...
    {
        $text = 'Dear Ankit kumar, Your Consutation has been booked on 2022/01/30 at 09:30 AM.89700 Kindly be at Clinic 5 mins earlier. Any Query, call - 24480874, 24484991.';
         $res =   $this->sendWhatsappNote(918057692119, $text);
         dd($res);
    ...
    public function updatePayment()
    {
        ...
        $table = $table->setTable('payment2022s');
        $tablename = $table->getTable();

        $itable =  new Invoices;
        $itable = $itable->setTable('invoice2022s');
        $itablename = $itable->getTable();

        $allinvoices =  $itable->select("$itablename.*")->get();
        $count = 0;
        foreach ($allinvoices as $key => $invoice) {
            DB::table($tablename)->insert([
                'admin_id' => $invoice->admin_id,
                'location_id' => $invoice->location_id,
                'amount' => ($invoice->payable_amount)?$invoice->payable_amount:0.000,
                'txnid' => $invoice->txnid,
                'invoice_no' =>  $invoice->invoice_number,
                'remark' => $invoice->remark,
                'status_id' => 2,
                'visa_amount' => $invoice->visa_amount,
                'cash_amount' => $invoice->cash_amount,
                'received' => $invoice->received,
                'returnable' => $invoice->returnable,
                'mode' => ($invoice->payment_mode)?$invoice->payment_mode:'unknown',

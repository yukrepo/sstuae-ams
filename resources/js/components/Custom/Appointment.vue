<template>
    <div>
        <div>
            <button class="box" @click="makeAppointment">
                <i class="fas fa-plus"></i>
            </button>
        </div>
        <div class="modal fullbarheadermodal fade" id="addApponitPopModal"  data-backdrop="static" data-keyboard="false" aria-labelledby="addApponitPopModalTitle" aria-hidden="true">
            <div class="modal-dialog modal-lg" role="document">
                <div class="modal-content">
                    <form @submit.prevent="createPopupAppointment()" class="popup-form">
                        <div class="modal-header">
                            <h5 class="modal-title" id="createAppointModalTitle">Create Appointment</h5>
                            <button type="button" class="btn btn-wide btn-sm btn-outline-light" @click="resetForm"> Reset Form </button>
                        </div>
                            ...
                            <div class="row">
                                <div class="col-md-8 col-xs-12">
                                    ...
                                    <div class="row">
                                            ...
                                            <div class="row">
                                                ...
                                                <div class="col-md-6">
                                                    <div class="input-group">
                                                        <input type="text" class="form-control" :class="{ 'is-invalid': cform.errors.has('course_id') }" v-model="cform.course_id" placeholder="Enter Course ID, If any">
                                                        <div class="input-group-append">
                                                            <img class="wf-50" :src="loaderurl" alt="updating" v-if="loader == 2">
                                                            <button class="btn btn-sm btn-outline-secondary p-t-1 p-b-0" @click="checkCourse" type="button" v-else> <i class="fas fa-search"></i> </button>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                            <div class="alert alert-info py-1" v-show="cmessage">
                                                {{ cmessage }}
                                            </div>
                                            <div class="row" v-show="paform.date">
                                                <div class="form-group col-md-9 col-xs-9" v-if="paform.user_id">
                                                ...
                                                <div class="form-group col-md-9 col-xs-9" v-else>
                                                    <label for="user_id" class="control-label">Search patient</label>
                                                    <div class="input-group">
                                                        <input type="text" class="form-control" v-model="sform.user_id" placeholder="User ID/ Mobile/First Name">
                                                        <div class="input-group-append">
                                                            <img class="wf-50" :src="loaderurl" alt="updating" v-if="loader == 1">
                                                            <button class="btn btn-sm btn-outline-secondary p-t-1 p-b-0" @click="checkUser" type="button" v-else> <i class="fas fa-search"></i> </button>
                                                        </div>
                                                    </div>
                                                    <has-error :form="sform" field="user_id"></has-error>
                                                </div>
                                                <div class="form-group col-md-3 col-xs-3">
                                                    <label class="control-label m-b-0">View</label><br>
                                                    <a class="btn btn-success view-btn" v-if="sselected != ''" href="javascript:;" @click="viewCustomer(paform.user_id)"> <i class="fas fa-eye"></i></a>
                                                    <a class="btn btn-secondary view-btn btn-block" v-else href="javascript:;"> <i class="fas fa-times"></i></a>
                                                    <a class="btn btn-dark view-btn" v-show="sselected != ''" href="javascript:;" @click="hideCustomer"> <i class="fas fa-eye-slash"></i></a>
                                                </div>
                                                <div class="form-group col-md-12 col-xs-12" v-show="((sform.user_id) && (customers.length >= 1) && (paform.user_id === ''))">
                                                    <table class="table table-bordered table-condensed qtable">
                                                        <thead>
                                                            <tr>
                                                                <th>User ID / Name</th>
                                                                <th>Mobile / Email</th>
                                                        ...
                                                        <tbody>
                                                            <tr v-for="user in customers" :key="user.id">
                                                                <td>{{ user.username }}<br>{{ user.first_name }} {{ user.last_name }}</td>
                                                                <td>{{ user.mobile }} <br>{{ user.email }}</td>
                                                                <td>
                                                                    <button class="btn btn-sm btn-primary" type="button" @click="onCustomerSelect(user.id)">
                                                ...
                                                <div class="form-group col-md-4 col-xs-12" v-show="paform.user_id">
                                                ...
                                                <div class="form-group col-md-8 col-xs-12" v-show="paform.user_id">
                                        ...
                                        <div class="col-md-6 col-sm-6 col-xs-12 border-right">
                                            <div class="row" v-show="(paform.date && paform.visit_type_id )">
                                    ...
                                    <div class="m-b-10 p-b-5 m-l-0 m-r-0 table-responsive" v-show="sselected">
                                        <table class="table table-bordered table-condensed table-striped qtable">
                                            <thead>
                                                <tr>
                                                    <th>SNo</th>
                                                    <th>Location</th>
                                        ...
                                        <button class="btn btn-md btn-warning" type="button" @click="paform.notify = 1" v-if="paform.notify == 0">Notify patient by SMS/Whatsapp</button>
                                        <button class="btn btn-md btn-success" type="button" @click="paform.notify = 0" v-else>Patient will be notified by SMS/Whatsapp</button>
                                    </div>
                                </div>
...
export default {
    ...
    methods: {
        ...
        updateTCList() {
            axios.get('/api/getConsultationsList?vtype='+this.paform.visit_type_id).then((data) => {this.consultations = data.data }).catch();
        },
        isExist(arr, cid) {
        ...
        checkUser()
        {
            this.loader = 1;
            this.customers = '';
            this.sform.post('/api/find-customer')
            .then((response) => {
                if(response.data) {
                    this.loader = false;
                    this.customers = response.data;
                }
                else { this.catchmessage = 'No Customer Found'; }
        ...
        checkCourse()
        {
            this.loader = 2;
            this.cmessage = '';
            this.cform.post('/api/courses/check-course')
            .then((response) => {
                if(response.data) {
                    this.loader = false;
                    this.cmessage = 'Course Type: '+response.data.type+' | Status: '+response.data.status;
                    this.paform.course_id = response.data.course_code;
                    this.paform.user_id = response.data.user_id;
                    this.onCustomerSelect(response.data.user_id);
                }
                else { this.cmessage = 'No Course Found'; }

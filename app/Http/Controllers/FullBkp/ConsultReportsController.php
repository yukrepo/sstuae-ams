...
class ConsultReportsController extends Controller
{
    ...
    public function consultationPrescriptionDayReport($date = null)
    {
        ...
        $record = $table->where('appointment_type_id', 1)->where('location_id', Auth::user()->location_id)->where("date", date('Y-m-d', strtotime($date)))->whereIn('status_id', [5,8,10])->orderBy('doctor_id', 'asc')->get();
    ...
    public function consultationPrescriptionMonthReport($date = null)
    {
        ...
        $record = $table->where('appointment_type_id', 1)->where('location_id', Auth::user()->location_id)->whereMonth('date', date('m', strtotime($date)))
    ...
    public function consultationPrescriptionCustomReport($sdate = null, $edate = null)
    {
        ...
        $sep = array(' ', ' ', ' ', ' ');
        fputcsv($output, $sep);
        $columns = array('S.No.', 'Appointment ID', 'Doctor', 'Customer ID', 'Customer Name', 'Customer Mobile No', 'Customer Nationality', 'Consultation', 'Symptoms', 'General Examination', 'Lab Test', 'Diagnosis', 'Therapies', 'Medicines', 'Appointment Invoice', 'Medicine Invoice', 'Course ID');
        fputcsv($output, $columns);
        ...
        $record = $table->where('appointment_type_id', 1)->where('location_id', Auth::user()->location_id)->whereDate('date','<=', date('Y-m-d', strtotime($edate)))
        ...
        foreach($record as $key => $index)
        {
            ...
            fputcsv($output, array(++$key, $index->appointment_code, $index->doctor->name, $index->user->username, $index->user_profile->first_name, $index->user->mobile, $index->user_profile->nationality->nationality, $index->treatment->treatment, $symptoms, $ge, $lt, $diagnosis, $therapies, $medicines, $index->ainvoice, $index->pinvoice, $index->course_id));
        }
    ...
    public function treatmentDrbasedDayReport($date = null)
    {
        ...
        if(Auth::user()->admin_type_id == 6):
            $doctors = Doctors::where('id', Auth::user()->relative_id)->where('designation_type_id', 2)->where('status_id', 2)->orderBy('gender', 'asc')->get();
        else:
        ...
        foreach($doctors as $doctor)
        {
            ...
            $columns = array('S.No.', 'Treatment', 'Total Count As Primary', 'Total Count As Secondary');
            fputcsv($output, $columns);
            ...
            foreach($data as $key => $index)
            {
                ...
                if(($record+$record2) >= 1)
                {
                    fputcsv($output, array(++$key, ucwords($index->treatment), $record, $record2));
                }
